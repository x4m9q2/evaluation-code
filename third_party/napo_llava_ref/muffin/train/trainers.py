...
def forward_DPO(model, input_ids, labels, attention_mask, images, **kwargs):
    ...
    if token_weighted:
        token_log_prob = impl(output.logits, labels, return_per_token_logp=True)
...
def get_beta_and_logps(data_dict, model, args, is_minicpm=False, is_llava15=False):
    ...
    if args.dpo_token_weighted:
        ...
        concatenated_logp = compute_weighted_logp(
            concatenated_logp,
            concatenated_labels,
            concatenated_token_weight,
            args.dpo_use_average,
        )
...
class LLaVA15DPOTrainer(ZephyrTrainer):
    ...
    def compute_loss(
        ...
        losses, chosen_rewards, rejected_rewards = dpo_loss(
            policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, beta=beta
        )
