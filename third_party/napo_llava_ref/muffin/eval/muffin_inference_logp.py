...
from muffin.train.train_utils import (
    encode_multimodal_preference_sample,
    SFT_collator_fn,
    preprocess_v1,
)
...
def image_field_to_pil(image_field):
    if isinstance(image_field, str):
        return PIL_image.open(image_field).convert("RGB")
    if isinstance(image_field, dict):
        if image_field.get("bytes") is not None:
            return bytes_to_PIL_image(image_field["bytes"])
        if image_field.get("path"):
            return PIL_image.open(image_field["path"]).convert("RGB")
    raise ValueError(f"Unsupported image field: {type(image_field)}")


def get_batch_logps_minicpm(
...
def get_batch_logps(
    ...
    ), f"logits.shape[:-1]={logits.shape[:-1]}, labels.shape={labels.shape}"
...
class PreferenceInferenceDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(origin_split, str):
            try:
                origin_split = json.loads(origin_split)
            except json.JSONDecodeError:
                # Some locally-built datasets store a plain split name like "train".
                pass
        metainfo = {
        ...
        image = image_field_to_pil(sample["image"])
...
def pretty_print(data_dict, tokenizer):
    ...
    for i, o in zip(input_ids, label_ids):
        ...
        o_tok = (
            tokenizer.convert_ids_to_tokens(o.item()) if o.item() != -100 else "[SKIP]"
        )
...
def write_logp_to_preference_parquet(
    origin_data, cache_file, logps, overwrite_logps=False
):
    out_data = []
    ...
    if torch.distributed.get_rank() == 0:
        ...
        for idx, start in enumerate(range(0, len(out_data), step)):
            ...
            df.to_parquet(
                os.path.join(
                    cache_file,
                    f"RLAIF-V-Dataset-withlogp_{idx:03}-{len(temp_data)}.parquet",
                )
            )
...
def inference_logp(
    ...
    num_workers = int(os.environ.get("MUFFIN_LOGP_NUM_WORKERS", "8"))
