...
RUN_NAME="${RUN_NAME:-gemma3_4b_pretrain_gate_projector_l1_sdpa}"
OUTPUT_DIR="${OUTPUT_DIR:-${PRETRAIN_CHECKPOINT}}"
...
"${PYTHON_BIN}" -m deepspeed.launcher.runner --include "localhost:${CUDA_VISIBLE_DEVICES}" src/train/train_sft.py \
  ...
  --gate_l1_loss_weight "${GATE_L1_LOSS_WEIGHT:-0.1}" \
