...
run_or_echo "${PYTHON_BIN}" -m deepspeed.launcher.runner llava/train/train_xformers.py \
  ...
  --patch_mask_analysis_path "${LLAVA_PATCH_MASK_NPZ}" \
  ...
  --bf16 True \
  --output_dir "${LLAVA_STAGE2_CHECKPOINT}" \
