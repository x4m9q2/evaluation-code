...
LLAVA_PRETRAIN_NOGATE_OUTPUT="${LLAVA_PRETRAIN_NOGATE_OUTPUT:-${BUNDLE_ROOT}/checkpoints/llava_pretrain_nogate}"
...
run_or_echo "${PYTHON_BIN}" -m deepspeed.launcher.runner llava/train/train_xformers.py \
  ...
  --gate_l1_loss_weight 0 \
  --mm_vision_select_layer -2 \
  --mm_use_im_start_end False \
  --mm_use_im_patch_token False \
  --bf16 True \
  --output_dir "${LLAVA_PRETRAIN_NOGATE_OUTPUT}" \
