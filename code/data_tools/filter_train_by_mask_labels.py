...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Filter train samples by question-object occlusion labels under a masked-image setting."
    )
    ...
    parser.add_argument(
        ...
        help="Labels to remove from the output train set.",
    )
    parser.add_argument(
        "--output-json",
...
def main() -> None:
    ...
    for idx, item in enumerate(train, start=1):
        ...
        if args.progress_every > 0 and idx % args.progress_every == 0:
            print(
                f"processed={idx} dropped={len(dropped)} "
