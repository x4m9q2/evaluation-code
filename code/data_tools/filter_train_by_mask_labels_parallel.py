...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Parallel filter for train samples by question-object occlusion labels."
    )
...
def init_worker(coco_instances: str, mask_dir: str, image_dir: str) -> None:
    global ALL_CATS, ANNS_BY_IMAGE, MASK_DIR, IMAGE_DIR
    ALL_CATS, ANNS_BY_IMAGE = load_coco(Path(coco_instances))
...
def mention_categories(question: str) -> list[str]:
    q = question.lower()
    hits = []
    for cat in ALL_CATS:
...
def classify_sample(item: dict) -> tuple[str, dict | None]:
...
def main() -> None:
    ...
    with ProcessPoolExecutor(
        ...
        initargs=(args.coco_instances, args.mask_dir, args.image_dir),
    ) as executor:
        for idx, (status, result) in enumerate(executor.map(classify_sample, train, chunksize=64), start=1):
            if status == "missing":
                missing_qids.add(int(train[idx - 1]["question_id"]))
            else:
                classified_by_qid[int(result["question_id"])] = result
                label_counter[result["label"]] += 1
                for reason in result["reasons"]:
                    reason_counter[reason] += 1
            if args.progress_every > 0 and idx % args.progress_every == 0:
                print(
                    f"processed={idx} missing={len(missing_qids)} "
    ...
    for item in train:
        qid = int(item["question_id"])
        result = classified_by_qid.get(qid)
        if result is None:
    ...
    report = {
        ...
        "missing_mask_or_image": len(missing_qids),
