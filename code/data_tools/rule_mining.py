...
except ImportError:  # pragma: no cover - optional dependency for match_rules
    torch = None
    DEVICE = None
...
def match_rules(
    ...
    if torch is None or DEVICE is None:
        raise ImportError("match_rules requires PyTorch. Please install torch to use this function.")
    ...
    transactions_matrix = torch.from_numpy(transactions_matrix).bool().to(DEVICE)
