...
def train():
    ...
    if not training_args.lora_enable and not getattr(training_args, "disable_ref_model", False):
    ...
    elif getattr(training_args, "disable_ref_model", False):
        rank0_print("[debug] disable_ref_model=True: reference log-probs will use the policy model context.")
    ...
    if ref_model is not None:
        configure_llm(ref_model, training_args)
        configure_vision_tower(ref_model, training_args, compute_dtype, training_args.device)
        ref_gate_text_tokenizer = configure_dual_input_gate(ref_model, training_args, compute_dtype)
        if training_args.use_dual_input_gate:
            maybe_restore_dual_input_gate_from_checkpoint(ref_model, model_args.model_id)
        if gate_text_tokenizer is None:
            gate_text_tokenizer = ref_gate_text_tokenizer
        ref_model.eval()
    ...
    skip_final_save = os.environ.get("SKIP_FINAL_SAVE", "0").lower() in ("1", "true", "yes")
    ...
    if skip_final_save:
        if local_rank == 0 or local_rank == -1:
            print("[train_dpo] SKIP_FINAL_SAVE=1: skipped trainer state and final model save.")
