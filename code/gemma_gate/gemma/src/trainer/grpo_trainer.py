...
class GemmaGRPOTrainer(Trainer):
    def __init__(
        self,
        ...
        if self.beta == 0.0:
            ...
            self.ref_model = Gemma3ForConditionalGeneration.from_pretrained(
                model_id,
                **model_init_kwargs,
            )
    ...
    def _generate_and_score_completions(
        ...
        for p in prompts:
            image_inputs = process_vision_info(p)
        ...
        token_type_ids = torch.zeros_like(
            prompt_completion_ids, dtype=torch.long, device=prompt_completion_ids.device
        )
        token_type_ids[prompt_completion_ids == self.processing_class.image_token_id] = 1

        multimodal_inputs = {k: prompt_inputs[k] if k in prompt_inputs else None for k in MULTIMODAL_KEYWORDS}
        multimodal_inputs['token_type_ids'] = token_type_ids
    ...
    def compute_liger_loss(self, model, inputs):
        ...
        token_type_ids[input_ids == self.processing_class.image_token_id] = 1
