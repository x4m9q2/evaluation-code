...
import torch
from transformers import BitsAndBytesConfig, Gemma3ForConditionalGeneration, AutoProcessor, AutoConfig
import os
import json
import warnings
import os
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, 
    ...
    if 'lora' in model_name.lower() and model_base is not None:
        ...
        print('Loading Gemma3 from base model...')
        model = Gemma3ForConditionalGeneration.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs)
        ...
        if model.language_model.lm_head.weight.shape[0] != token_num:
            ...
            model.language_model.embed_tokens.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=model.device, dtype=model.dtype))

        print('Loading additional Gemma3 weights...')
        ...
        model = Gemma3ForConditionalGeneration.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
