...
def _resolve_checkpoint_path(
    checkpoint_path: Optional[str], load_from_HF: bool
) -> Optional[str]:
    """
    Resolve checkpoint source for SAM3 model loading.

    Priority:
      1) Explicit `checkpoint_path` argument
      2) `SAM3_CHECKPOINT_PATH` environment variable
      3) Hugging Face download when `load_from_HF=True`
    """
    if checkpoint_path is not None:
        return checkpoint_path

    env_checkpoint_path = os.getenv("SAM3_CHECKPOINT_PATH")
    if env_checkpoint_path:
        if not g_pathmgr.exists(env_checkpoint_path):
            raise FileNotFoundError(
                f"SAM3_CHECKPOINT_PATH is set but does not exist: {env_checkpoint_path}"
            )
        return env_checkpoint_path
...
def build_sam3_image_model(
    ...
    Args:
        ...
        checkpoint_path: Optional path to model checkpoint. If None, will try
            SAM3_CHECKPOINT_PATH from environment before downloading from HF.
...
def build_sam3_video_model(
    ...
    Args:
        checkpoint_path: Optional path to checkpoint file
            If None, will try SAM3_CHECKPOINT_PATH from environment before
            downloading from HF.
