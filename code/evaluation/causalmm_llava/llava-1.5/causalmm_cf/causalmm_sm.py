...
def causalmm_sample(
    ...
    **model_kwargs,
    ...
    while True:
        ...
        # Implementing counterfactual reasoning through contrastive decoding, following the setting of VCD
        next_token_logits = outputs.logits[:, -1, :]
        
        next_token_logits_cf = cf_outputs.logits[:, -1, :]
        ...
        cutoff = torch.log(torch.tensor(epsilon)) + next_token_logits.max(dim=-1, keepdim=True).values
        
        diffs = (1 + gamma) * next_token_logits - gamma * next_token_logits_cf

        cf_logits = diffs.masked_fill(next_token_logits < cutoff, -float("inf"))

        cf_logits = logits_processor(input_ids, cf_logits)
        cf_logits = logits_warper(input_ids, cf_logits)

        next_token_scores = cf_logits
        cf_probs = nn.functional.softmax(cf_logits, dim=-1)
        next_tokens = torch.multinomial(cf_probs, num_samples=1).squeeze(1)
        ...
        model_kwargs_cf = self._update_model_kwargs_for_generation(
            cf_outputs, model_kwargs_cf, is_encoder_decoder=self.config.is_encoder_decoder
        )
