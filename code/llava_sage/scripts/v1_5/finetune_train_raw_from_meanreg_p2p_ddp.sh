...
REPO_ROOT=${REPO_ROOT:-/path/to/local_scratch/LLaVA}
cd "${REPO_ROOT}"
...
TRAIN_RAW_JSON=${TRAIN_RAW_JSON:-/path/to/local_scratch/LLaVA/train_raw.json}
TRAIN_JSON=${TRAIN_JSON:-/path/to/local_scratch/LLaVA/playground/data/train_raw_llava_train2017.json}
IMAGE_FOLDER=${IMAGE_FOLDER:-/path/to/local_scratch/LLaVA/playground/data/coco}
VISION_TOWER=${VISION_TOWER:-/path/to/local_scratch/LLaVA/clip-vit-large-patch14-336}
BASE_MODEL=${BASE_MODEL:-/path/to/local_scratch/LLaVA/llava-v1.5-7b}
PRETRAIN_ADAPTER=${PRETRAIN_ADAPTER:-/path/to/local_scratch/LLaVA/checkpoints/llava_pretrain_4gpu_xformers_aggressive_p2p_bs32_gate_pretain_meanreg_20260319_052641/mm_projector.bin}

REPORT_TO=${REPORT_TO:-wandb}
RUN_TAG=${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}
OUTPUT_DIR=${OUTPUT_DIR:-/path/to/local_scratch/LLaVA/checkpoints/finetune_train_raw_from_meanreg_p2p_ddp_${RUN_TAG}}
...
if [ ! -f "${TRAIN_JSON}" ]; then
  python /path/to/local_scratch/LLaVA/playground/data/convert_train_raw_to_llava.py \
    --input "${TRAIN_RAW_JSON}" \
    --output "${TRAIN_JSON}" \
    --image-subdir train2017
fi
