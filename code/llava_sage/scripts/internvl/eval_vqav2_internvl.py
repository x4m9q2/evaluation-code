...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run InternVL2.5-8B on VQAv2 test-dev style JSONL data."
    )
    parser.add_argument("--model-path", required=True)
    parser.add_argument("--question-file", required=True, help="VQAv2 JSONL/JSON file.")
    parser.add_argument("--image-folder", required=True, help="Folder containing test2015 images.")
...
class VQAv2Dataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        image_name = str(item["image"]).strip()
