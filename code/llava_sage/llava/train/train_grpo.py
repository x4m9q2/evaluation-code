...
from llava.train.grpo_dataset import make_grpo_data_module
from llava.train.grpo_trainer import LLaVAGRPOTrainer
...
class GRPOTrainingArguments(transformers.TrainingArguments):
    ...
    grpo_lambda_ori: float = field(default=1.0)
    grpo_lambda_as: float = field(default=1.2)
    grpo_lambda_sep: float = field(default=0.5)
    grpo_lambda_cross: float = field(default=0.5)
