...
def main() -> None:
    ...
    if len(gpu_list) == 1:
        ...
        env["PYTHONPATH"] = (
            f"/path/to/sage_repro_bundle{os.pathsep}{env['PYTHONPATH']}"
            if env.get("PYTHONPATH")
            else "/path/to/sage_repro_bundle"
        )
        cmd = base_cmd + ["--answers-file", str(answers_file)]
        print(f"[run] {model_tag}", flush=True)
        subprocess.run(cmd, cwd="/path/to/sage_repro_bundle", env=env, check=True)
        ...
        for chunk_idx, gpu_id in enumerate(gpu_list):
            ...
            procs.append(subprocess.Popen(cmd, cwd="/path/to/sage_repro_bundle", env=env))
            cmds.append(cmd)
